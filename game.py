...
class screen:
    ...
    def assets_load(self):
        ...
        self.method = pygame.image.load('assets/search_method.png')
    ...
    def draw(self, win):
        ...
        elif self.state == State.METHOD_CHOOSING:
            win.blit(self.method,(0,0))
...
class game:
    ...
    def main(self):
        ...
        while(run):
            ...
            if self.scr.state == State.MAIN_MENU:
            ...
            elif self.scr.state ==State.METHOD_CHOOSING:
                for event in events:
                    if event.type  == pygame.KEYUP:
                        if event.key == pygame.K_LEFT:
                            self.scr.state = State.WAIT_FOR_SEARCHING
                            self.chosen_method = 'bfs'
                            self.chosen = True
                        elif event.key == pygame.K_RIGHT:
                            self.scr.state = State.WAIT_FOR_SEARCHING
                            self.chosen_method = 'astar'
                            self.chosen = True
                        elif event.key == pygame.K_z:
                            if self.chosen_set == 'mini':
                                self.scr.state = State.MINI_CHOOSING
                            elif self.chosen_set == 'micro':
                                self.scr.state = State.MICRO_CHOOSING
                            self.chosen = False
                    elif event.type == pygame.MOUSEBUTTONUP:
                        pos = pygame.mouse.get_pos()
                ...
                if self.chosen:
                    ...
                    if self.chosen_method == 'astar':
                        res = sokoban.astar_search()
                    elif self.chosen_method == 'bfs':
                        res = sokoban.blind_search()
                    if res == 'timeout':
